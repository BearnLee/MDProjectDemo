...
import afinal.annotation.view.EventListener;
...
public class FinalActivity extends Activity {
    ...
    public static void initInjectedView(Object injectedSource,Class<?> clazz,View sourceView){
		...
		if(fields!=null && fields.length>0){
			for(Field field : fields){
				...
				if(viewInject!=null){
					...
					if(viewId == 0){
						String idString = viewInject.idString();
						if(!TextUtils.isEmpty(idString)){
							viewId = sourceView.getContext().getResources().getIdentifier(
									idString, 
									"id", 
